...
export default function MostWanted({ onSelectProperty }: MostWantedProps) {
  ...
  // Use native scroll on mobile OR when user prefers reduced motion
  const useNativeScroll = isMobile || shouldReduceMotion;
  ...
  useEffect(() => {
    // Skip calculation on mobile — native scroll handles it
    if (!trackRef.current || useNativeScroll) return;

    const calculateScroll = () => {
      if (trackRef.current) {
        const translateVal = trackRef.current.scrollWidth - window.innerWidth;
  ...
  }, [mostWantedList.length, useNativeScroll]);
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // MOBILE: Native browser horizontal scroll — buttery smooth, browser-threaded
  // This is the ONLY way to get truly lag-free horizontal scroll on mobile.
  // ─────────────────────────────────────────────────────────────────────────
  if (useNativeScroll) {
    return (
      <section
      ...
      >
        ...
        <div className="px-6 mb-8">
          ...
          <p className="text-[10px] font-mono text-neutral-400 tracking-widest uppercase mt-2">
            ← Deslizá para explorar →
          </p>
        </div>

        {/* Native horizontal scroll container — browser handles this on its own thread */}
        <div
          className="overflow-x-auto no-scrollbar px-6 pb-6"
        ...
        >
          <div className="flex gap-4 w-max pr-6">
            {mostWantedList.map((property) => (
              <div
                key={property.id}
                className="shrink-0 w-[78vw] h-[460px] rounded-3xl overflow-hidden bg-white/80 border border-neutral-200/80 premium-card-shadow relative cursor-pointer"
              ...
              >
                ...
                <div className="absolute top-5 right-5 z-20">
                  <span className="text-xs px-3 py-1.5 bg-white/95 border border-neutral-200/60 font-mono text-neutral-900 rounded-xl shadow-md font-medium">
                    {property.price}
                  </span>
                </div>

                {/* Bottom info panel */}
                <div className="absolute bottom-5 left-5 right-5 z-20 p-5 rounded-2xl bg-white border border-neutral-200/80 premium-card-shadow text-neutral-900">
                  <div className="flex items-center justify-between gap-3 mb-1">
                    <h3 className="text-lg font-display font-medium tracking-tight leading-tight">
                      {property.title}
                    </h3>
                    <span className="shrink-0 w-8 h-8 rounded-xl bg-neutral-50 border border-neutral-200 flex items-center justify-center text-neutral-600">
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // DESKTOP: Scroll-driven sticky horizontal parallax (Framer Motion)
  // ─────────────────────────────────────────────────────────────────────────
  return (
    <div
      ref={targetRef}
      id="mas-cotizadas"
      className="relative dynamic-light-lilac-gradient border-y border-neutral-200/50 h-[300vh]"
    >
      <div className="sticky top-0 h-screen flex flex-col justify-center overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-12 mb-10 w-full flex items-end justify-between gap-6 z-10 shrink-0">
          ...
          <div className="flex flex-col items-end gap-1.5 font-mono text-[10px] text-neutral-500 font-light">
            <span className="tracking-widest uppercase">Deslizá hacia abajo para explorar</span>
          </div>
        </div>
