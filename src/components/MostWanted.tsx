...
export default function MostWanted({ onSelectProperty }: MostWantedProps) {
  ...
  if (isMobile) {
    return (
      <div
        id="mas-cotizadas"
        className="relative w-full py-16 dynamic-light-lilac-gradient border-y border-neutral-200/50"
      >
        <div className="relative w-full flex flex-col justify-center">
          ...
          {/* Horizontal track carrying items - native horizontal swipe on mobile */}
          <div 
            className="relative w-full overflow-x-auto snap-x snap-mandatory no-scrollbar px-6 pb-8 pt-4 flex gap-6"
            style={{ WebkitOverflowScrolling: 'touch', touchAction: 'pan-x' }}
          >
            {mostWantedList.map((property) => (
              <div
                key={property.id}
                className="shrink-0 w-[85vw] sm:w-[58vw] max-w-[600px] h-[480px] rounded-3xl overflow-hidden bg-white/70 backdrop-blur-md border border-neutral-200/80 premium-card-shadow relative group cursor-pointer hover:border-emerald-500/30 transition-all snap-center hover:-translate-y-1"
  ...
  // Desktop sticky vertical-to-horizontal scrolling
  return (
    <div
      ref={targetRef}
      id="mas-cotizadas"
      ...
      <div className="sticky top-0 h-screen flex flex-col justify-center overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-12 mb-10 w-full flex items-end justify-between gap-6 z-10 shrink-0">
          ...
          <div className="flex flex-col items-end gap-1.5 font-mono text-[10px] text-neutral-500 font-light">
            <span className="tracking-widest uppercase">Desplázate hacia abajo para explorar</span>
          </div>
        </div>
